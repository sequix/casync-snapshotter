...
// Umount will be returned by various mount functions to provides a non-blocking mount call.
type Umount interface {
	// MountErrChan returns a channel which provide the error returned by syscall.Mount().
	MountErrChan() chan error
	// Umount performs actual umount and any necessary clean actions.
	Umount() error
}
